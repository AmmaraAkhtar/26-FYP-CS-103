...
class BrowsingMonitoringScreen extends StatefulWidget {
  final int childId;
  final Map<String, dynamic>? childData;
  const BrowsingMonitoringScreen({super.key, required this.childId, this.childData});

  @override
  State<BrowsingMonitoringScreen> createState() => _BrowsingMonitoringScreenState();
}

class _BrowsingMonitoringScreenState extends State<BrowsingMonitoringScreen> {
  List<dynamic> browsingList = [];
  ...
  Future<void> _fetchData() async {
    ...
    final data = await DashboardService.getBrowsingData(widget.childId, currentFilter);
    if (mounted) {
      if (data != null) {
        setState(() {
          browsingList = data['browsing'] ?? [];
            ...
            children: [
              ...
              Container(
                ...
                child: Row(
                  mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    _buildTab("Today", isSelected: currentFilter == 'today', onTap: () {
                      setState(() => currentFilter = 'today');
                      _fetchData();
                    }),
                    _buildDivider(),
                    _buildTab("This Week", isSelected: currentFilter == 'week', onTap: () {
                      setState(() => currentFilter = 'week');
                      _fetchData();
                    }),
                    _buildDivider(),
                    _buildTab("This Month", isSelected: currentFilter == 'month', onTap: () {
                      setState(() => currentFilter = 'month');
                      _fetchData();
                    }),
                    _buildDivider(),
                    Icon(Icons.more_horiz, color: Colors.grey, size: 20.sp),
              ...
              isLoading
                  ...
                  : browsingList.isEmpty
                      ? Padding(
                          padding: EdgeInsets.symmetric(vertical: 20.h),
                          child: Center(
                            child: Text("No browsing data", style: TextStyle(color: Colors.grey, fontSize: 13.sp)),
                          ),
                        )
                      : Column(
                          children: browsingList.map<Widget>((item) {
                            final bool blocked = item['blocked'] == true;
                            final String category = item['category'] ?? 'Unknown';
                            return _buildMonitoringRow(
                              item['title'] ?? '',
                              "${item['time_spent'] ?? 0}s",
                              category,
                              blocked ? Colors.red : Colors.green,
                              Icons.public,
                              blocked ? Colors.red : Colors.blue,
                              blocked,
                            );
                          }).toList(),
                        ),
              ...
              isLoading
                  ...
                  : alertsList.isEmpty
                      ? Padding(
                          ...
                          child: Text("No alerts", style: TextStyle(color: Colors.grey, fontSize: 13.sp)),
                        )
                      : Column(
                          children: alertsList.map<Widget>((alert) {
  ...
  // Helper Widget for Browsing Rows
  Widget _buildMonitoringRow(String title, String time, String category, Color catColor,
      IconData icon, Color iconColor, bool blocked) {
    return Card(
      elevation: 2,
      margin: EdgeInsets.symmetric(vertical: 8.h),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(12.r)),
  ...
  // Helper Widget for Safety Alerts
  Widget _buildAlertTile(String type, String message, String time) {
    return Card(
      elevation: 2,
      margin: EdgeInsets.only(bottom: 12.h),
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15.r)),
      child: ListTile(
        contentPadding: EdgeInsets.symmetric(horizontal: 12.w, vertical: 4.h),
        leading: Icon(Icons.error, color: Colors.red, size: 35.sp),
        title: Text(type, style: TextStyle(fontSize: 13.sp, fontWeight: FontWeight.w500)),
        subtitle: Text(
          message,
          style: TextStyle(color: Colors.redAccent, fontSize: 12.sp),
          maxLines: 1,
        ...
        trailing: Text(time, style: TextStyle(fontSize: 9.sp, color: Colors.grey)),
      ),
    );
  }
}
